...
The browser-side widget is used for interaction, while R is used for reproducible
...
`safePivot` supports table and heatmap renderers, readable heatmap palettes, and
...
`safe_pivot_compute_from_config()` also accepts default rows, columns, values,
and aggregator settings. These defaults are important because `input$pivot_config`
may be `NULL` before the widget sends its first configuration back to Shiny.
...
current configuration and recomputes the result in R.
