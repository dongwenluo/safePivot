...
This design avoids scraping the HTML table from the browser. Instead, the browser sends the pivot configuration back to Shiny, and R recomputes the exported table from the original data.
...
A browser pivot table is interactive. Users can move variables between rows, columns, and values, and they can change the aggregator.

Instead of trying to parse the displayed HTML table, `safePivot` captures the current configuration and recomputes the result in R.
...
If a variable is an R factor, `safePivot` can respect the factor-level order.
...
`safePivot` sends data to the browser for interactive pivoting. For large data, filter or aggregate first before sending data to `safePivot()`.
